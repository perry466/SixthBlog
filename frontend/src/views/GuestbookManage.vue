<template>
  <div class="manage-page">
    ...
    <div v-else-if="list.length === 0" class="empty">暂无留言</div>
...
import { getAdminGuestbook, updateGuestbook, deleteGuestbook } from '../api/blog'
...
const fetchList = async () => {
  ...
  try {
    list.value = await getAdminGuestbook(params) as any[]
  } catch { showToast('获取留言列表失败', 'error') }
  finally { loading.value = false }
}

const approve = async (id: number) => {
  try {
    await updateGuestbook(id, { is_approved: true })
    showToast('留言已通过')
    fetchList()
...
const reject = async (id: number) => {
  try {
    await updateGuestbook(id, { is_approved: false })
    showToast('留言已驳回')
    fetchList()
  } catch { showToast('操作失败', 'error') }
}
...
const doDelete = async () => {
  try {
    await deleteGuestbook(deleteTarget.value)
    showToast('留言已删除')
